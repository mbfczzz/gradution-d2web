  ...
  <div>
   ...
   <el-table
    ...
    <template slot-scope="scope">
        ...
        <span style="margin-left: 10px">{{ scope.row.assigementTime}}</span>
    </template>    
    </el-table-column>
     <el-table-column
        label="分配人"
...
export default {
  components: { PageInfo,edit,add},
    name:"tableMain",
    props:['tdata','statusNames','taskNames'],
    data(){
        return {
          id:[]
        }
    ...
    /**
      ...
      deleteUser:async function () {
       ...
       const res = await api.DELETE_ASSIGMENT(this.id)
        if(res.code ===200){
          Message({
            showClose: true,
            message: res.message,
      ...
      handleEdit:function(index,row){
        ...
        this.$refs.edit.form.taskId = row.taskId
        ...
        this.$refs.edit.tmp.taskId = row.taskId
      ...
      handleDelete: async function(index,row){
        let arr=[]
        arr.push(row.id)
        const res = await api.DELETE_ASSIGMENT(arr)
        if(res.code ===200){
        Message({
            showClose: true,
            message: res.message,
