<template>
  <div>
   ...
   <el-table
    ...
    <template slot-scope="scope">
        <el-tag
        v-if="scope.row.refundDescribe"
        effect="plain"
        type="success"
        >
        {{ scope.row.refundDescribe}}
        </el-tag>
    </template>  
    </el-table-column>   
    <el-table-column
  ...
  props:['tdata'],
    ...
    /**
      ...
      deleteUser:async function () {
       ...
       const res = await api.DELETE_REFUNDWAY(this.id)
        if(res.code ===200){
          Message({
            showClose: true,
            message: res.message,
      ...
      handleEdit:function(index,row){
        ...
        this.$refs.edit.form.refundName = row.refundName
        this.$refs.edit.form.refundDescribe = row.refundDescribe 
        this.$refs.edit.tmp.id = row.id
        this.$refs.edit.tmp.refundName = row.refundName
        this.$refs.edit.tmp.refundDescribe = row.refundDescribe 
      },   
      /**
       * 单个删除
       */
      handleDelete: async function(index,row){
        let arr=[]
        arr.push(row.id)
        const res = await api.DELETE_REFUNDWAY(arr)
        if(res.code ===200){
        Message({
            showClose: true,
            message: res.message,
