    ...
    methods:{
     resetPage:function(data){
      ...
      deleteUser:async function () {
       ...
       const res = await api.DELETE_POSITION(this.id)
        if(res.code ===200){
          Message({
            showClose: true,
            message: res.message,
      ...
      handleEdit:function(index,row){
        ...
        this.$refs.edit.title = "修改职位信息"
        this.$refs.edit.form.id = row.id
        this.$refs.edit.form.positionName =row.positionName
        this.$refs.edit.tmp.id = row.id
        this.$refs.edit.tmp.positionName = row.positionName                      
      },
      ...
      handleDelete: async function(index,row){
        let arr=[]
        arr.push(row.id)
        const res = await api.DELETE_POSITION(arr)
        if(res.code ===200){
        Message({
            showClose: true,
            message: res.message,
