<template>
  <div>
     <el-card>
     <el-button type="primary" @click="handleadd">添加标准</el-button>
...
export default {
  components: { PageInfo,edit,add},
    name:"tableMain",
    props:['tdata','standardTypes','rewardDescribes'],
    data(){
        return {
          id:[],
        }
    ...
    methods:{
     resetPage:function(data){
      ...
      deleteUser:async function () {
       ...
       const res = await api.DELETE_STANDARD(this.id)
        if(res.code ===200){
          Message({
            showClose: true,
            message: res.message,
     ...
     switchBoolByIsValid:async function(index,row){
      ...
      handleEdit:function(index,row){
        ...
        this.$refs.edit.form.standardName = row.standardName
        this.$refs.edit.form.standardType = row.standardType
        this.$refs.edit.form.rewardContent = row.rewardContent
        this.$refs.edit.form.rewardDescribe = row.rewardDescribe
        this.$refs.edit.tmp.id = row.id
        this.$refs.edit.tmp.standardName = row.standardName
        this.$refs.edit.tmp.standardType = row.standardType
        this.$refs.edit.tmp.rewardContent = row.rewardContent
        this.$refs.edit.tmp.rewardDescribe = row.rewardDescribe                            
      },
      /**
       * 单个删除
       */
      handleDelete: async function(index,row){
        let arr=[]
        arr.push(row.id)
        const res = await api.DELETE_STANDARD(arr)
        if(res.code ===200){
        Message({
            showClose: true,
            message: res.message,
